...
if (isset($_GET['id_loaisp'])) {
    ...
    $sql_count = "SELECT COUNT(*) AS total FROM sanpham WHERE id_loaisp = $id";
    ...
    $offset = ($page - 1) * $limit;
    $sql = "SELECT * FROM sanpham WHERE id_loaisp = $id LIMIT $offset, $limit";
...
?>
    ...
    if ($total_pages > 1) {
        ...
        <div class="pagination">
            <?php if ($page > 1) { ?>
                <a href="./index.php?id_loaisp=<?php echo $id; ?>&page=<?php echo $page - 1; ?>">Trang trước</a>
            <?php } ?>
            <?php for ($i = 1; $i <= $total_pages; $i++) { ?>
                <?php if ($i == $page) { ?>
                    <a class="active"><?php echo $i; ?></a>
                <?php } else { ?>
                    <a href="./index.php?id_loaisp=<?php echo $id; ?>&page=<?php echo $i; ?>"><?php echo $i; ?></a>
                <?php } ?>
            <?php } ?>
            <?php if ($page < $total_pages) { ?>
                <a href="./index.php?id_loaisp=<?php echo $id; ?>&page=<?php echo $page + 1; ?>">Trang sau</a>
            <?php } ?>
        </div>
<?php
    }
